 ...
 * 描述:慕课网下拉刷新风格
 */
public class BQMoocStyleRefreshViewHolder extends BQRefreshViewHolder {
    private BQMoocStyleRefreshView mMoocRefreshView;
    private int mUltimateColorResId = -1;
    private int mOriginalImageResId = -1;

    public BQMoocStyleRefreshViewHolder(Context context, boolean isLoadingMoreEnabled) {
    ...
    public View getRefreshHeaderView() {
        if (mRefreshHeaderView == null) {
            ...
            mMoocRefreshView = (BQMoocStyleRefreshView) mRefreshHeaderView.findViewById(R.id.moocView);
            if (mOriginalImageResId != -1) {
                mMoocRefreshView.setOriginalImage(mOriginalImageResId);
            } else {
                throw new RuntimeException("请调用" + BQMoocStyleRefreshViewHolder.class.getSimpleName() + "的setOriginalImage方法设置原始图片资源");
            }
            if (mUltimateColorResId != -1) {
                mMoocRefreshView.setUltimateColor(mUltimateColorResId);
            } else {
                throw new RuntimeException("请调用" + BQMoocStyleRefreshViewHolder.class.getSimpleName() + "的setUltimateColor方法设置最终生成图片的填充颜色资源");
            }
        }
