...
public class BQMeiTuanRefreshViewHolder extends BQRefreshViewHolder {
    private BQMeiTuanRefreshView mMeiTuanRefreshView;
     ...
     */
    public BQMeiTuanRefreshViewHolder(Context context, boolean isLoadingMoreEnabled) {
    ...
    public View getRefreshHeaderView() {
        if (mRefreshHeaderView == null) {
            ...
            mMeiTuanRefreshView = (BQMeiTuanRefreshView) mRefreshHeaderView.findViewById(R.id.meiTuanView);
            if (mPullDownImageResId != -1) {
                mMeiTuanRefreshView.setPullDownImageResource(mPullDownImageResId);
            } else {
                throw new RuntimeException("请调用" + BQMeiTuanRefreshViewHolder.class.getSimpleName() + "的setPullDownImageResource方法设置下拉过程中的图片资源");
            }
            if (mChangeToReleaseRefreshAnimResId != -1) {
                ...
                throw new RuntimeException("请调用" + BQMeiTuanRefreshViewHolder.class.getSimpleName() + "的setChangeToReleaseRefreshAnimResId方法设置进入释放刷新状态时的动画资源");
            }
            if (mRefreshingAnimResId != -1) {
                mMeiTuanRefreshView.setRefreshingAnimResId(mRefreshingAnimResId);
            } else {
                throw new RuntimeException("请调用" + BQMeiTuanRefreshViewHolder.class.getSimpleName() + "的setRefreshingAnimResId方法设置正在刷新时的动画资源");
            }
        }
