 ...
 */
public class BQRefreshLayout extends LinearLayout {
    private static final String TAG = BQRefreshLayout.class.getSimpleName();

    private BQRefreshViewHolder mRefreshViewHolder;
    /**
    ...
    private BQStickyNavLayout mStickyNavLayout;
    ...
    public BQRefreshLayout(Context context) {
        this(context, null);
    }

    public BQRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(BQRefreshLayout.class.getSimpleName() + "必须有且只有一个子控件");
        }
        ...
        } else if (mContentView instanceof BQStickyNavLayout) {
            mStickyNavLayout = (BQStickyNavLayout) mContentView;
    ...
    public void setRefreshViewHolder(BQRefreshViewHolder refreshViewHolder) {
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        ...
        return BQRefreshScrollingUtil.isAbsListViewToBottom(absListView);
    }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        ...
        return BQRefreshScrollingUtil.isRecyclerViewToBottom(recyclerView);
    }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (BQRefreshScrollingUtil.isWebViewToBottom(mWebView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isScrollViewToBottom(mScrollView)) {
            return true;
        }
    ...
    private boolean isContentViewToTop() {
        ...
        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mWebView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mScrollView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isAbsListViewToTop(mAbsListView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isRecyclerViewToTop(mRecyclerView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isStickyNavLayoutToTop(mStickyNavLayout)) {
            return true;
        }
    ...
    private void showLoadingMoreView() {
        ...
        BQRefreshScrollingUtil.scrollToBottom(mScrollView);
        BQRefreshScrollingUtil.scrollToBottom(mRecyclerView);
        BQRefreshScrollingUtil.scrollToBottom(mAbsListView);
        if (mStickyNavLayout != null) {
    ...
    public interface RefreshLayoutDelegate {
         ...
         * 开始刷新
         */
        void onRefreshLayoutBeginRefreshing(BQRefreshLayout refreshLayout);
         ...
         */
        boolean onRefreshLayoutBeginLoadingMore(BQRefreshLayout refreshLayout);
    }
