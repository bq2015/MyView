 ...
 * 描述:滑动时固定导航栏
 */
public class BQStickyNavLayout extends LinearLayout {
    ...
    public BQRefreshLayout mRefreshLayout;

    public BQStickyNavLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 3) {
            throw new IllegalStateException(BQStickyNavLayout.class.getSimpleName() + "必须有且只有三个子控件");
        }
    ...
    public boolean isContentViewToTop() {
        ...
        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectWebView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectScrollView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isAbsListViewToTop(mDirectAbsListView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isRecyclerViewToTop(mDirectRecyclerView)) {
            return true;
        }
    ...
    private boolean isViewPagerContentViewToTop() {
        ...
        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedWebView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedScrollView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isAbsListViewToTop(mNestedAbsListView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isRecyclerViewToTop(mNestedRecyclerView)) {
            return true;
        }
    ...
    private void regetNestedContentView() {
        ...
        if (adapter instanceof FragmentPagerAdapter || adapter instanceof FragmentStatePagerAdapter) {
            ...
            throw new IllegalStateException(BQStickyNavLayout.class.getSimpleName() + "的第三个子控件为ViewPager时，其adapter必须是FragmentPagerAdapter或者FragmentStatePagerAdapter");
        }
    }

    public void setRefreshLayout(BQRefreshLayout refreshLayout) {
    ...
    public boolean shouldHandleLoadingMore() {
        ...
        if (BQRefreshScrollingUtil.isWebViewToBottom(mDirectWebView)) {
            return true;
        }

        if (BQRefreshScrollingUtil.isScrollViewToBottom(mDirectScrollView)) {
            return true;
        }
        ...
        if (mDirectViewPager != null) {
            ...
            if (BQRefreshScrollingUtil.isWebViewToBottom(mNestedWebView)) {
                return true;
            }

            if (BQRefreshScrollingUtil.isScrollViewToBottom(mNestedScrollView)) {
                return true;
            }
    ...
    public void scrollToBottom() {
        BQRefreshScrollingUtil.scrollToBottom(mDirectScrollView);
        BQRefreshScrollingUtil.scrollToBottom(mDirectRecyclerView);
        BQRefreshScrollingUtil.scrollToBottom(mDirectAbsListView);

        if (mDirectViewPager != null) {
            if (mNestedContentView == null) {
                regetNestedContentView();
            }
            BQRefreshScrollingUtil.scrollToBottom(mNestedScrollView);
            BQRefreshScrollingUtil.scrollToBottom(mNestedRecyclerView);
            BQRefreshScrollingUtil.scrollToBottom(mNestedAbsListView);
        }
    }
}